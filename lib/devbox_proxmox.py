...
def qaexec(vmid: int,cmd = 'uptime',node: str = node):
  ...
  try: 
    ...
    kmsg(kname, f'{vmid}: problem running cmd: {cmd}', 'err')
    print(qa_exec)
    exit(0)
  ...
  while pid_status != int(1):
    try:
      ...
      kmsg(kname, f'{vmid}: problem with pid: {pid} {cmd}', 'err')
      exit(0)
  ...
  if int(pid_check['exitcode']) == 127:
    kmsg(kname, f'{vmid}: exit code 127: {pid} {cmd}', 'err')
    exit(0)
...
def internet_check(vmid):
  ...
  if internet_check == 'error':
    kmsg('prox_netcheck', f'{vmnames[vmid]} internet access check failed', 'err')
    exit(0)
